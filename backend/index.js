...
const FRONTEND_BASE_URL = (process.env.FRONTEND_URL || process.env.CORS_ORIGIN || 'http://localhost:5173').split(',')[0].trim().replace(/\/$/, '');
...
const buildEmailContent = ({ type, payload, recipient }) => {
  ...
  const map = {
    ...
    'worker.created': {
      subject: 'Ваш акаунт у FinOK CRM створено',
      text: `Для вас створено акаунт у FinOK CRM.\n\nEmail: ${normalized?.email || recipient || '—'}\n${normalized?.password ? `Тимчасовий пароль: ${normalized.password}\n` : ''}${normalized?.loginUrl ? `Вхід: ${normalized.loginUrl}\n` : ''}${normalized?.message ? `\n${normalized.message}` : ''}`,
    },
    'auth.password-reset': {
      subject: 'Відновлення пароля FinOK',
      text: `Ми отримали запит на скидання пароля.\n\nПосилання для створення нового пароля: ${normalized?.resetLink || '—'}\nТермін дії посилання: ${normalized?.expiresIn || '15m'}\n\nЯкщо це були не ви, просто проігноруйте цей лист.`,
    },
    ...
    'payment.invoice.client': {
      subject: normalized?.subject || 'Рахунок від FinOK',
      text: normalized?.message || 'Надсилаємо вам рахунок у вкладенні.',
    },
    'payment.receipt.client': {
      subject: normalized?.subject || 'Квитанція від FinOK',
      text: normalized?.message || 'Надсилаємо вам квитанцію у вкладенні.',
    },
    'payment.contract.client': {
      subject: normalized?.subject || 'Договір від FinOK',
      text: normalized?.message || 'Надсилаємо вам договір у вкладенні.',
    },
  };
...
const documentTypeConfig = {
  invoice: {
    ...
    auditAction: 'EXPORT_PAYMENT_INVOICE',
  },
  receipt: {
    ...
    auditAction: 'EXPORT_PAYMENT_RECEIPT',
  },
  contract: {
    type: 'payment.contract.client',
    title: 'Contract',
    prefix: 'CNT',
    ...
    auditAction: 'EXPORT_PAYMENT_CONTRACT',
  },
};
...
const queueNotification = async ({ consultationId = null, userId = null, channel, type, recipient = null, payload = null }) => {
  try {
    const payloadToStore = payload && typeof payload === 'object' && Array.isArray(payload.attachments)
      ? {
          ...payload,
...
const paymentDocumentSendSchema = z.object({
...
const getPaidConsultationForPaymentDocument = async ({ consultationId, user }) => {
  const consultation = await prisma.consultation.findFirst({
    where: {
      id: consultationId,
      consultationType: 'PAID',
      ...getManagerRoleFilter(user),
    },
    include: {
      service: true,
      order: true,
...
const buildPaymentDocumentBundle = ({ consultation, documentType }) => {
  const config = documentTypeConfig[documentType];
  if (!config) throw new AppError(400, 'Unsupported payment document type');
  ...
  const documentNumber = `${config.prefix}-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${consultation.id.slice(-6).toUpperCase()}`;
  const managerName = consultation.assignedManager?.firstName || consultation.assignedManager?.email || null;
  const fileName = `${config.fileNamePrefix}-${consultation.id}.pdf`;
...
const generatePaymentDocumentPdf = async ({ consultation, documentType }) => {
  const bundle = buildPaymentDocumentBundle({ consultation, documentType });
  const pdfBuffer = await createPaymentPdf({
    documentTitle: bundle.config.title,
    documentNumber: bundle.documentNumber,
    payment: bundle.payment,
    consultation,
    managerName: bundle.managerName,
    footerNote: bundle.config.footerNote,
  });
...
app.post('/api/auth/request-password-reset', authLimiter, validateBody(forgotPasswordSchema), asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const resetLink = `${FRONTEND_BASE_URL}/reset-password?token=${encodeURIComponent(resetToken)}`;
...
app.post('/api/admin/workers/invite', authMiddleware, requireRole(['ADMIN']), validateBody(workerInviteSchema), asyncHandler(async (req, res) => {
  ...
  const inviteLink = `${FRONTEND_BASE_URL}/register?invite=${encodeURIComponent(inviteToken)}&email=${encodeURIComponent(email)}`;

  const inviteNotification = await queueNotification({
    channel: 'email',
    type: 'worker.invite',
    recipient: email,
    payload: {
  ...
  res.status(201).json({ email, role, inviteLink, expiresIn: '7d', inviteEmailStatus: inviteNotification?.status || 'queued' });
}));
...
app.post('/api/admin/workers', authMiddleware, requireRole(['ADMIN']), validateBody(workerCreateSchema), asyncHandler(async (req, res) => {
  ...
  const welcomeNotification = await queueNotification({
    userId: worker.id,
    channel: 'email',
    type: 'worker.created',
    ...
    payload: {
      email: worker.email,
      password: password ? null : tempPassword,
      loginUrl: `${FRONTEND_BASE_URL}/login`,
      message: 'Увійдіть у систему та змініть пароль після першого входу.',
    },
  });

  res.status(201).json({ ...worker, generatedPassword: password ? null : tempPassword, welcomeEmailStatus: welcomeNotification?.status || 'queued' });
}));
...
app.get('/api/admin/payments/:consultationId/invoice.pdf', authMiddleware, requireRole(['ADMIN', 'MANAGER', 'ACCOUNTANT', 'VIEWER']), asyncHandler(async (req, res) => {
  if (!canManagePayments(req.user.role)) throw new AppError(403, 'Forbidden');
  const { consultationId } = req.params;
  const consultation = await getPaidConsultationForPaymentDocument({ consultationId, user: req.user });
  const document = await generatePaymentDocumentPdf({ consultation, documentType: 'invoice' });

  await logAudit({
    actorUserId: req.user.id,
    consultationId: consultation.id,
    entityType: document.config.auditEntityType,
    entityId: document.documentNumber,
    action: document.config.auditAction,
    metadata: {
...
app.get('/api/admin/payments/:consultationId/receipt.pdf', authMiddleware, requireRole(['ADMIN', 'MANAGER', 'ACCOUNTANT', 'VIEWER']), asyncHandler(async (req, res) => {
  ...
  const { consultationId } = req.params;
  const consultation = await getPaidConsultationForPaymentDocument({ consultationId, user: req.user });
  const document = await generatePaymentDocumentPdf({ consultation, documentType: 'receipt' });
  ...
  await logAudit({
    ...
    entityId: document.documentNumber,
    action: document.config.auditAction,
    metadata: { amount: document.amount, currency: 'UAH', consultationId: consultation.id },
  });
...
app.get('/api/admin/payments/:consultationId/contract.pdf', authMiddleware, requireRole(['ADMIN', 'MANAGER', 'ACCOUNTANT', 'VIEWER']), asyncHandler(async (req, res) => {
  ...
  const { consultationId } = req.params;
  const consultation = await getPaidConsultationForPaymentDocument({ consultationId, user: req.user });
  const document = await generatePaymentDocumentPdf({ consultation, documentType: 'contract' });
...
app.post('/api/admin/payments/:consultationId/send-document', authMiddleware, requireRole(['ADMIN', 'MANAGER', 'ACCOUNTANT']), validateBody(paymentDocumentSendSchema), asyncHandler(async (req, res) => {
  if (!canManagePayments(req.user.role)) throw new AppError(403, 'Forbidden');
  const { consultationId } = req.params;
  const { documentType, recipient, subject, message } = req.body;

  const consultation = await getPaidConsultationForPaymentDocument({ consultationId, user: req.user });
  const document = await generatePaymentDocumentPdf({ consultation, documentType });
  const finalRecipient = recipient || consultation.email;

  if (!finalRecipient) throw new AppError(400, 'Recipient email is required');
  ...
  await logAudit({
    ...
    entityType: document.config.auditEntityType,
    entityId: document.documentNumber,
    action: `EMAIL_${document.config.auditAction}`,
    metadata: {
      consultationId: consultation.id,
      recipient: finalRecipient,
